...
try {
  await client.query(`set statement_timeout = '${Number.parseInt(process.env.AUDIT_TIMEOUT_MS ?? '120000', 10)}'`);
  ...
  const multiCve = await client.query(`
      with suspicious as (
        select canonical_vulnerability_id, count(distinct normalized_value)::bigint cve_count
